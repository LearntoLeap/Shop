...
async function loadProduct() {
  ...
  if (!ident) {
    main.innerHTML = notFoundHtml('Thiếu mã sản phẩm.');
    return;
  }
  try {
    ...
    // If user landed on a non-canonical URL, replace history with canonical
    const canonical = productUrl(p);
    if (location.pathname !== '/Shop/' + canonical && !location.search.includes('id=')) {
      // optional: history.replaceState({}, '', canonical);
    }
    render(p, data);
  } catch (e) {
    console.error(e);
    main.innerHTML = notFoundHtml('Không tải được dữ liệu sản phẩm.');
  }
}
...
function notFoundHtml(msg) {
  return `<div class="text-center py-20">
    ...
    <div class="text-slate-600 mb-4">${msg}</div>
...
function render(p, data) {
  ...
  document.getElementById('productMain').innerHTML = `
    <nav class="text-sm text-slate-500 mb-5 flex items-center gap-2 flex-wrap">
      <a href="index.html" class="hover:text-brand-700">Trang chủ</a>
      <span>›</span>
      ${cat ? `<a href="index.html" class="hover:text-brand-700">${cat.icon || ''} ${cat.name}</a><span>›</span>` : ''}
      ...
      <div>
        ...
        <div class="space-y-2 mb-5 text-sm">
          <div class="flex items-center gap-2">
            <span class="text-slate-500 w-28">Tình trạng:</span>
            ${p.stock > 0
        ...
        <a href="mailto:contact@learntoleap.vn?subject=${encodeURIComponent('Liên hệ đặt sản phẩm: ' + p.name + (p.sku ? ' (' + p.sku + ')' : ''))}&body=${encodeURIComponent('Tôi quan tâm đến sản phẩm: ' + p.name + (p.sku ? '\nMã: ' + p.sku : ''))}"
    ...
    <div class="mt-10">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ${related.map(p => {
          ...
          return `
            <a href="${productUrl(p)}" class="block bg-white rounded-lg border border-slate-100 hover:shadow-md transition overflow-hidden">
